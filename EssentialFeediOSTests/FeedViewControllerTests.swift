...
final class FeedViewControllerTests: XCTestCase {
    ...
    func test_feedView_hasTitle() {
        ...
        XCTAssertNotEqual(localizedTitle, localizedKey, "Expected the localised title to have a value that is not the localised key: \(localizedKey). The key/value pair is missing from the table.")
    ...
    func test_LoadFeedActions_requestsFeedFromLoader() {
        ...
        sut.simulateUserInitiatedFeedReload()
        XCTAssertEqual(loader.loadFeedCallCount, 3, "Expected a third loading request once user intiiates another load.")
    }
    ...
    class LoaderSpy: FeedLoader, FeedImageDataLoader {
        ...
        // MARK: - FeedLoader
        private var loadFeedCompletions = [(LoadFeedResult) -> Void]()
        
        var loadFeedCallCount: Int {
            return loadFeedCompletions.count
        }
        
        func load(completion: @escaping (LoadFeedResult) -> Void) {
            loadFeedCompletions.append(completion)
        }
        
        func completeFeedLoading(with feed: [FeedImage] = [], at index: Int = 0) {
            loadFeedCompletions[index](.success(feed))
        }
        
        func completeFeedLoadingWithError(at index: Int = 0) {
            let error = NSError(domain: "an error", code: 0)
            loadFeedCompletions[index](.failure(error))
        }
